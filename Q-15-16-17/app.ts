let guest_list: string[] = [ `Zain`, `Iqrash`, `Nomam`, `Samad` ];
// for(let i=0; i<guest_list.length; i++){
//     console.log( ` Dear ` + guest_list[i] + `\n\nYou are invited for a dinner tomorrow.\n\n Thank you`);
    
// }

let not_coming: string = "Zain";
let new_guest: string = "Mavia";
guest_list[0] = new_guest
// for(let i=0; i<guest_list.length; i++){
//     console.log( ` Dear ` + guest_list[i] + `\n\nYou are invited for a dinner tomorrow.\n\n Thank you\n\n`);
    
// }

// console.log(`Mr ${not_coming}, is not coming for dinner tomorrow.\n` );

guest_list.unshift(` Babar Azam `);
guest_list.splice(guest_list.length/2, 2, "Shahid Afridi" );
guest_list.push("Muhammad Amir");
// for(let i=0; i<guest_list.length; i++){
//     console.log( ` Dear ` + guest_list[i] + `\n\nYou are invited for a dinner tomorrow.\n\n Thank you\n\n`);
    
// }
// console.log(`We have found a bigger dinner talbe so we invited more guests`);

console.log(`\n\nUnfortunately we are out of space, so have space for two guest only.\n\n`);
while(guest_list.length>2){
    let removed_guest = guest_list.pop();
    console.log(`Sorry, Dear ${removed_guest}, You are not invited for a dinner tomorrow`);
    
}
for(let i=0; i<guest_list.length; i++){
    console.log( ` Dear ` + guest_list[i] + `\n\nYou all are still invited for a dinner tomorrow.\n\n Thank you\n\n`);
    
}

guest_list.splice(0,2);
console.log(guest_list);
